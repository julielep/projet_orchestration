...
* [x] **SQLite de test** : Développez votre module `sqlalchemy` pour qu'il fonctionne d'abord sur une base SQLite locale.
* [x] **API FastAPI** : Créez deux routes : `POST /data` (sauvegarde) et `GET /data` (récupération).
* [x] **Logique métier** : séparez vos codes dans votre dossier API (maths, connexion, crud, data)
* [x] **Frontend Streamlit** : Concevez les deux pages demandées.
* [x] **Tests** : Validez votre API avec Pytest (maths et api).
...
🗒️Notes : 
- pyproject.toml -> met à la racine du projet pour que les tests reconnaissent app_api comme chemin 
- Attention aux chemins relatifs, on se met toujours depuis la racine du projet
